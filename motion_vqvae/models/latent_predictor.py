"""
Latent Predictor for VQ-VAE auxiliary loss.
Predicts future codebook sequences to learn temporal dynamics in latent space.
...
class LatentPredictor(nn.Module):
    """
    Predictor that forecasts future codebook sequences from current sequences.
    ...
    Output: predicted codebook logits [batch_size, pred_len, num_codebooks]
    """
    ...
    def __init__(
        ...
        num_layers: int = 2,
        dropout: float = 0.1,
        pred_len: int = 1,  # Number of future steps to predict
        ...
        if self.architecture == "mlp":
            ...
            # Output layer: predict next codebook indices
            layers.append(nn.Linear(input_dim, num_codebooks * pred_len))
            ...
            self.predictor = nn.Sequential(
                ...
                nn.ReLU(),
                nn.Dropout(dropout),
                nn.Linear(hidden_dim, num_codebooks * pred_len)
            )
            self.transformer = None
            self.cnn1d = None
            self.pos_encoding = None
    ...
    def forward(self, codebook_seq: torch.Tensor) -> torch.Tensor:
        ...
        Returns:
            pred_logits: Predicted codebook logits [batch_size, pred_len, num_codebooks]
        """
        ...
        if self.architecture == "mlp":
            ...
            # Predict: [batch_size, code_dim] -> [batch_size, num_codebooks * pred_len]
            pred_logits_flat = self.predictor(pooled)
            ...
            pooled = lstm_out[:, -1, :]
            # Predict: [batch_size, hidden_dim] -> [batch_size, num_codebooks * pred_len]
            pred_logits_flat = self.predictor(pooled)
        ...
        # Reshape to [batch_size, pred_len, num_codebooks]
        pred_logits = pred_logits_flat.view(batch_size, self.pred_len, self.num_codebooks)
        
        return pred_logits
    
    def predict(self, codebook_seq: torch.Tensor) -> torch.Tensor:
        """
        Predict future codebook indices (hard predictions).
        
        Args:
            codebook_seq: Current codebook indices [batch_size, seq_len]
        
        Returns:
            pred_indices: Predicted codebook indices [batch_size, pred_len]
        """
        logits = self.forward(codebook_seq)
        return torch.argmax(logits, dim=-1)  # [batch_size, pred_len]
