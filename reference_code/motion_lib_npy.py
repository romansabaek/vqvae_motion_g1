...
class MotionLibNpy:
    def __init__(self, motion_file, device, motion_id=None):
        ...
        self._motion_file_path = motion_file
    ...
    def _load_motions(self, motion_file):
        ...
        self._motion_tokens = []  # Stores VQVAE token values from npy file
        ...
        self._actual_motion_ids = []  # Store actual motion IDs (83, 201, etc.) from npy file
        ...
        # Always use the last column as motion_id (ignore any extra columns)
        motion_identifiers = trajectory_data[:, -1]
        
        # 2. Split the trajectory into individual motion clips based on the motion identifier
        
        # Detect split points where motion_identifier changes
        # This happens when we transition from one motion to the next
        if len(motion_identifiers) > 1:
            # Find indices where motion_identifier changes (i.e., motion_id[i] != motion_id[i+1])
            changes = np.where(motion_identifiers[:-1] != motion_identifiers[1:])[0] + 1
            split_indices = [0] + changes.tolist()
        else:
            split_indices = [0]
        
        # Debug: Log motion identifier statistics
        unique_ids, counts = np.unique(motion_identifiers, return_counts=True)
        logger.debug(f"Motion identifiers: unique values={unique_ids.tolist()}, counts={counts.tolist()}, "
                    f"detected {len(split_indices)-1} transition(s), resulting in {len(split_indices)} motion clip(s)")

        # Extract actual motion IDs from filename (e.g., saved_desired_states_83_201_fg.npy -> [83, 201])
        import re
        import os
        motion_basename = os.path.splitext(os.path.basename(motion_file))[0]
        filename_numeric_ids = [int(m) for m in re.findall(r'\d+', motion_basename) if m.isdigit()]
        
        motion_clips = []
        actual_motion_ids_list = []
        num_clips = len(split_indices)
        ...
        for i in range(num_clips):
            ...
            # Map clip index to actual motion ID from filename, or use clip index as fallback
            actual_motion_id = filename_numeric_ids[i] if i < len(filename_numeric_ids) else i
            actual_motion_ids_list.append(actual_motion_id)
        ...
        if self._motion_id_to_load is not None:
            ...
            self._actual_motion_ids = [actual_motion_ids_list[self._motion_id_to_load]]
        else:
            motions_to_process = {f"motion_{i}": clip for i, clip in enumerate(motion_clips)}
            self._actual_motion_ids = actual_motion_ids_list.copy()

        # 3. Process each segmented motion clip
        for idx, (name, motion_data_raw) in enumerate(tqdm(motions_to_process.items(), desc="[MotionLib] Processing motions")):
            try:
                ...
                # Extract DOF positions: handle token column if present
                # Format: [time, root_pos(3), root_rot(4), dof_pos(23), token?, motion_id]
                # If 33 cols: token column exists at index -2; if 32 cols: no token column
                num_cols = motion_data_raw.shape[1]
                if num_cols == 33:
                    # Has token column: [time, root_pos(3), root_rot(4), dof_pos(23), token(1), motion_id(1)]
                    dof_pos = torch.tensor(motion_data_raw[:, 8:31], dtype=torch.float, device=self._device)
                else:
                ...
                # Extract tokens if present (column 31 in 33-col format, otherwise zeros)
                num_cols = motion_data_raw.shape[1]
                if num_cols == 33:
                    # Has token column: [time, root_pos(3), root_rot(4), dof_pos(23), token(1), motion_id(1)]
                    motion_tokens = torch.tensor(motion_data_raw[:, 31], dtype=torch.float, device=self._device)
                else:
                    # No token column: use zeros
                    motion_tokens = torch.zeros(num_frames, dtype=torch.float, device=self._device)
                
                curr_len = dt * (num_frames - 1)

                transition_flags_clip = torch.zeros(num_frames, dtype=torch.float, device=self._device)
                ...
                self._motion_tokens.append(motion_tokens)
        ...
        self._motion_tokens = torch.cat(self._motion_tokens, dim=0)
        ...
        self._actual_motion_ids = torch.tensor(self._actual_motion_ids, dtype=torch.long, device=self._device)
        
        total_len = self.get_total_length()
        print(f"Loaded and segmented {self.num_motions()} motions with a total length of {total_len:.3f}s.")
        print(f"Actual motion IDs: {self._actual_motion_ids.cpu().numpy().tolist()}")
    ...
    def get_motion_tokens(self, motion_ids, motion_times):
        """
        Get VQVAE token values for given motion_ids and motion_times.
        
        Args:
            motion_ids: (num_envs,) tensor of motion clip indices
            motion_times: (num_envs,) tensor of local times within each motion
        
        Returns:
            tokens: (num_envs,) tensor of token values
        """
        motion_loop_num = torch.floor(motion_times / self._motion_lengths[motion_ids])
        motion_times_clamped = motion_times - motion_loop_num * self._motion_lengths[motion_ids]
        frame_idx0, frame_idx1, blend = self._calc_frame_blend(motion_ids, motion_times_clamped)
        
        # Get tokens at frame indices
        token0 = self._motion_tokens[frame_idx0]
        token1 = self._motion_tokens[frame_idx1]
        
        # Interpolate tokens (though tokens are typically constant per motion, so this may just return token0)
        tokens = (1.0 - blend) * token0 + blend * token1
    ...
    def get_actual_motion_ids(self, motion_indices: torch.Tensor) -> torch.Tensor:
        """Convert motion clip indices (0, 1, 2, ...) to actual motion IDs (83, 201, ...)."""
        if not hasattr(self, '_actual_motion_ids') or self._actual_motion_ids is None:
            return motion_indices
        return self._actual_motion_ids[motion_indices]
